...
console.log('DB rawUrl:', rawUrl ? rawUrl.substring(0, 40) + '...' : 'EMPTY');
...
if (rawUrl.startsWith('libsql://') || rawUrl.startsWith('https://') || rawUrl.startsWith('http://')) {
  ...
  initDB = async () => {
    ...
    // Migration: allow NULL telegram_id (idempotent, ignore errors)
    try { await db.execute(`ALTER TABLE resellers ALTER COLUMN telegram_id DROP NOT NULL`); } catch (e) {}
    try { await db.execute(`CREATE UNIQUE INDEX IF NOT EXISTS idx_resellers_tg ON resellers(telegram_id) WHERE telegram_id IS NOT NULL`); } catch (e) {}
